...
class HorasTrabajadas(toga.App):
    def startup(self):
        ...
        self.box_inicio = toga.Box(style=Pack(direction=COLUMN, padding=10))
        self.box_historial_fecha = toga.Box(style=Pack(direction=COLUMN, padding=10))

        self.configurar_vista_inicio()
        self.configurar_vista_historial_fecha()
    ...
    def configurar_vista_inicio(self):
        self.box_inicio = toga.Box(style=Pack(direction=COLUMN, padding=10))
        ...
        historial_fecha_button = toga.Button('Historial por fecha', on_press=self.mostrar_vista_fecha, style=Pack(padding=5))

        self.box_inicio.add(self.hora_inicio_input)
        self.box_inicio.add(self.hora_fin_input)
        self.box_inicio.add(calcular_button)
        self.box_inicio.add(self.result_label)
        self.box_inicio.add(historial_button)
        self.box_inicio.add(self.result_label_historial)
        self.box_inicio.add(historial_fecha_button)

    def configurar_vista_historial_fecha(self):
        self.box_historial_fecha = toga.Box(style=Pack(direction=COLUMN, padding=10))

        self.por_fecha = toga.TextInput(placeholder="Fecha (YYYY-MM-DD)", style=Pack(padding=5))
        ...
        self.box_historial_fecha.add(self.por_fecha)
        self.box_historial_fecha.add(buscar_button)
        self.box_historial_fecha.add(volver_button)
        self.box_historial_fecha.add(self.resultado_por_fecha)

    def reiniciar_pantalla(self, widget=None):
        self.main_box.children.clear()

        box_inicio = toga.Box(style=Pack(direction=COLUMN, padding=10))
        ...
        box_inicio.add(self.hora_inicio_input)
        box_inicio.add(self.hora_fin_input)
        box_inicio.add(calcular_button)
        box_inicio.add(self.result_label)
        box_inicio.add(historial_button)
        box_inicio.add(self.result_label_historial)
        box_inicio.add(historial_fecha_button)

        self.main_box.add(box_inicio)

    def mostrar_vista_fecha(self, widget):
        self.main_box.children.clear()
    
        box_historial_fecha = toga.Box(style=Pack(direction=COLUMN, padding=10))
    
        self.por_fecha = toga.TextInput(placeholder="Fecha (YYYY-MM-DD)", style=Pack(padding=5))
        resultado_por_fecha = toga.Label("", style=Pack(padding=5))
    
        def buscar(widget):
            fecha = self.por_fecha.value.strip()
            if not fecha:
                resultado_por_fecha.text = "Ingresá una fecha válida."
                return
            ...
            if not rows:
                resultado_por_fecha.text = f"No hay registros para {fecha}"
                return
            ...
            resultado_por_fecha.text = f"Horas trabajadas el {fecha}: {total}\n\n{detalle}"
    
        buscar_button = toga.Button('Buscar', on_press=buscar, style=Pack(padding=5))
        volver_button = toga.Button('Volver al inicio', on_press=self.reiniciar_pantalla, style=Pack(padding=5))
    
        box_historial_fecha.add(self.por_fecha)
        box_historial_fecha.add(buscar_button)
        box_historial_fecha.add(volver_button)
        box_historial_fecha.add(resultado_por_fecha)
    
        self.main_box.add(box_historial_fecha)
...
#                continue
        ...
        self.result_label_historial.text = f"Horas trabajadas en el mes: {result}"
